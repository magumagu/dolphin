...
void CCodeWindow::StepOver()
{
	if (CCPU::IsStepping())
	{
		UGeckoInstruction inst = Memory::Debug_Read_Instruction(PC);
		if (inst.LK)
		{
			PowerPC::breakpoints.ClearAllTemporary();
...
void CCodeWindow::StepOut()
{
	if (CCPU::IsStepping())
	{
		...
		while (inst.hex != 0x4e800020 && steps < timeout) // check for blr
		{
			...
			inst = Memory::Debug_Read_Instruction(PC);
		}
...
void CCodeWindow::OnJitMenu(wxCommandEvent& event)
{
	switch (event.GetId())
	{
		...
		case IDM_SEARCH_INSTRUCTION:
		{
			...
			{
				const char *name = PPCTables::GetInstructionName(Memory::Debug_Read_U32(addr));
