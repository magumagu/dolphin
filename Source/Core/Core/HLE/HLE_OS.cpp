...
void HLE_GeneralDebugPrint()
{
	...
	if (PowerPC::Debug_Read_U32(GPR(3)) > 0x80000000)
	{
...
void GetStringVA(std::string& _rOutBuffer, u32 strReg)
{
	...
	std::string string = PowerPC::Debug_GetString(GPR(strReg));
	...
	for(u32 i = 0; i < string.size(); i++)
	{
		if (string[i] == '%')
		{
			...
			{
				Parameter = PowerPC::Debug_Read_U32(GPR(1) + 0x8 + ((ParameterCounter - 11) * 4));
			}
			else
			{
			...
			{
			case 's':
				_rOutBuffer += StringFromFormat(ArgumentBuffer.c_str(), PowerPC::Debug_GetString((u32)Parameter).c_str());
				break;
