...
{
	u32 temp = (u32)Memory::CPU_Read_U8(Helper_Get_EA(_inst));
...
void Interpreter::lbzu(UGeckoInstruction _inst)
{
	...
	u32 temp = (u32)Memory::CPU_Read_U8(uAddress);
	if (!(PowerPC::ppcState.Exceptions & EXCEPTION_DSI))
	{
		rGPR[_inst.RD] = temp;
...
{
	u64 temp = Memory::CPU_Read_U64(Helper_Get_EA(_inst));
	if (!(PowerPC::ppcState.Exceptions & EXCEPTION_DSI))
		riPS0(_inst.FD) = temp;
}
...
{
	u32 uAddress = Helper_Get_EA_U(_inst);
	u64 temp = Memory::CPU_Read_U64(uAddress);
	if (!(PowerPC::ppcState.Exceptions & EXCEPTION_DSI))
	{
		riPS0(_inst.FD) = temp;
		rGPR[_inst.RA]  = uAddress;
...
{
	u64 temp = Memory::CPU_Read_U64(Helper_Get_EA_X(_inst));
	if (!(PowerPC::ppcState.Exceptions & EXCEPTION_DSI))
		riPS0(_inst.FD) = temp;
}
...
{
	u32 uTemp = Memory::CPU_Read_U32(Helper_Get_EA(_inst));
	if (!(PowerPC::ppcState.Exceptions & EXCEPTION_DSI))
	{
...
{
	u32 uAddress = Helper_Get_EA_U(_inst);
	u32 uTemp = Memory::CPU_Read_U32(uAddress);
	if (!(PowerPC::ppcState.Exceptions & EXCEPTION_DSI))
	{
		u64 value = ConvertToDouble(uTemp);
		riPS0(_inst.FD) = value;
...
{
	u32 uTemp = Memory::CPU_Read_U32(Helper_Get_EA_X(_inst));
	if (!(PowerPC::ppcState.Exceptions & EXCEPTION_DSI))
	{
		u64 value = ConvertToDouble(uTemp);
		riPS0(_inst.FD) = value;
...
{
	u32 temp = (u32)(s32)(s16)Memory::CPU_Read_U16(Helper_Get_EA(_inst));
	if (!(PowerPC::ppcState.Exceptions & EXCEPTION_DSI))
	{
		rGPR[_inst.RD] = temp;
	}
...
{
	u32 uAddress = Helper_Get_EA_U(_inst);
	u32 temp = (u32)(s32)(s16)Memory::CPU_Read_U16(uAddress);
	if (!(PowerPC::ppcState.Exceptions & EXCEPTION_DSI))
	{
		rGPR[_inst.RD] = temp;
		rGPR[_inst.RA] = uAddress;
...
{
	u32 temp = (u32)(u16)Memory::CPU_Read_U16(Helper_Get_EA(_inst));
	if (!(PowerPC::ppcState.Exceptions & EXCEPTION_DSI))
	{
		rGPR[_inst.RD] = temp;
	}
...
{
	u32 uAddress = Helper_Get_EA_U(_inst);
	u32 temp = (u32)(u16)Memory::CPU_Read_U16(uAddress);
	if (!(PowerPC::ppcState.Exceptions & EXCEPTION_DSI))
	{
		rGPR[_inst.RD] = temp;
		rGPR[_inst.RA] = uAddress;
...
void Interpreter::lmw(UGeckoInstruction _inst)
{
	...
	{
		u32 TempReg = Memory::CPU_Read_U32(uAddress);
		if (PowerPC::ppcState.Exceptions & EXCEPTION_DSI)
		{
...
void Interpreter::stmw(UGeckoInstruction _inst)
{
	...
	{
		Memory::CPU_Write_U32(rGPR[iReg], uAddress);
		if (PowerPC::ppcState.Exceptions & EXCEPTION_DSI)
		{
...
{
	u32 uAddress = Helper_Get_EA(_inst);
	u32 temp = Memory::CPU_Read_U32(uAddress);
	if (!(PowerPC::ppcState.Exceptions & EXCEPTION_DSI))
	{
		rGPR[_inst.RD] = temp;
	}
...
{
	Memory::CPU_Write_U8((u8)rGPR[_inst.RS], Helper_Get_EA(_inst));
}

void Interpreter::stbu(UGeckoInstruction _inst)
{
	u32 uAddress = Helper_Get_EA_U(_inst);
	Memory::CPU_Write_U8((u8)rGPR[_inst.RS], uAddress);
	if (!(PowerPC::ppcState.Exceptions & EXCEPTION_DSI))
	{
		rGPR[_inst.RA] = uAddress;
	}
...
{
	Memory::CPU_Write_U64(riPS0(_inst.FS), Helper_Get_EA(_inst));
}

void Interpreter::stfdu(UGeckoInstruction _inst)
{
	u32 uAddress = Helper_Get_EA_U(_inst);
	Memory::CPU_Write_U64(riPS0(_inst.FS), uAddress);
	if (!(PowerPC::ppcState.Exceptions & EXCEPTION_DSI))
	{
		rGPR[_inst.RA] = uAddress;
	}
...
{
	Memory::CPU_Write_U32(ConvertToSingle(riPS0(_inst.FS)), Helper_Get_EA(_inst));
}

void Interpreter::stfsu(UGeckoInstruction _inst)
{
	u32 uAddress = Helper_Get_EA_U(_inst);
	Memory::CPU_Write_U32(ConvertToSingle(riPS0(_inst.FS)), uAddress);
	if (!(PowerPC::ppcState.Exceptions & EXCEPTION_DSI))
	{
		rGPR[_inst.RA] = uAddress;
	}
...
{
	Memory::CPU_Write_U16((u16)rGPR[_inst.RS], Helper_Get_EA(_inst));
}

void Interpreter::sthu(UGeckoInstruction _inst)
{
	u32 uAddress = Helper_Get_EA_U(_inst);
	Memory::CPU_Write_U16((u16)rGPR[_inst.RS], uAddress);
	if (!(PowerPC::ppcState.Exceptions & EXCEPTION_DSI))
	{
		rGPR[_inst.RA] = uAddress;
	}
...
{
	Memory::CPU_Write_U32(rGPR[_inst.RS], Helper_Get_EA(_inst));
}

void Interpreter::stwu(UGeckoInstruction _inst)
{
	u32 uAddress = Helper_Get_EA_U(_inst);
	Memory::CPU_Write_U32(rGPR[_inst.RS], uAddress);
	if (!(PowerPC::ppcState.Exceptions & EXCEPTION_DSI))
	{
		rGPR[_inst.RA] = uAddress;
	}
...
// 		_inst.RS, EA, PowerPC::ppcState.spr[SPR_EAR] & 0x1f);
	...
	rGPR[_inst.RS] = Memory::CPU_Read_U32(EA);
}
...
// 		rGPR[_inst.RS], EA, PowerPC::ppcState.spr[SPR_EAR] & 0x1f);
	...
	Memory::CPU_Write_U32(rGPR[_inst.RS], EA);
}
...
{
	u32 temp = (u32)Memory::CPU_Read_U8(Helper_Get_EA_X(_inst));
	if (!(PowerPC::ppcState.Exceptions & EXCEPTION_DSI))
	{
		rGPR[_inst.RD] = temp;
	}
...
{
	u32 uAddress = Helper_Get_EA_UX(_inst);
	s32 temp = (s32)(s16)Memory::CPU_Read_U16(uAddress);
	if (!(PowerPC::ppcState.Exceptions & EXCEPTION_DSI))
	{
		rGPR[_inst.RD] = temp;
		rGPR[_inst.RA] = uAddress;
...
{
	s32 temp = (s32)(s16)Memory::CPU_Read_U16(Helper_Get_EA_X(_inst));
	if (!(PowerPC::ppcState.Exceptions & EXCEPTION_DSI))
	{
		rGPR[_inst.RD] = temp;
	}
...
{
	u32 temp = (u32)Common::swap16(Memory::CPU_Read_U16(Helper_Get_EA_X(_inst)));
	if (!(PowerPC::ppcState.Exceptions & EXCEPTION_DSI))
	{
		rGPR[_inst.RD] = temp;
	}
...
{
	u32 uAddress = Helper_Get_EA_UX(_inst);
	u32 temp = (u32)Memory::CPU_Read_U16(uAddress);
	if (!(PowerPC::ppcState.Exceptions & EXCEPTION_DSI))
	{
		rGPR[_inst.RD] = temp;
		rGPR[_inst.RA] = uAddress;
...
{
	u32 temp = (u32)Memory::CPU_Read_U16(Helper_Get_EA_X(_inst));
	if (!(PowerPC::ppcState.Exceptions & EXCEPTION_DSI))
	{
		rGPR[_inst.RD] = temp;
	}
...
void Interpreter::lswx(UGeckoInstruction _inst)
{
	...
	if (n > 0)
	{
		...
		do
		{
			u32 TempValue = Memory::CPU_Read_U8(EA) << (24 - i);
			if (PowerPC::ppcState.Exceptions & EXCEPTION_DSI)
			{
...
{
	u32 temp = Common::swap32(Memory::CPU_Read_U32(Helper_Get_EA_X(_inst)));
	if (!(PowerPC::ppcState.Exceptions & EXCEPTION_DSI))
	{
		rGPR[_inst.RD] = temp;
	}
...
{
	Memory::CPU_Write_U8((u8)rGPR[_inst.RS], Helper_Get_EA_X(_inst));
}
...
{
	Memory::CPU_Write_U64(riPS0(_inst.FS), Helper_Get_EA_X(_inst));
}
...
void Interpreter::stfiwx(UGeckoInstruction _inst)
{
	...
	Memory::CPU_Write_U32((u32)riPS0(_inst.FS), uAddress);
}
...
{
	Memory::CPU_Write_U32(ConvertToSingle(riPS0(_inst.FS)), Helper_Get_EA_X(_inst));
}

void Interpreter::sthbrx(UGeckoInstruction _inst)
{
	Memory::CPU_Write_U16(Common::swap16((u16)rGPR[_inst.RS]), Helper_Get_EA_X(_inst));
}
...
{
	Memory::CPU_Write_U16((u16)rGPR[_inst.RS], Helper_Get_EA_X(_inst));
}
...
void Interpreter::stswi(UGeckoInstruction _inst)
{
	...
	while (n > 0)
	{
		...
		Memory::CPU_Write_U8((rGPR[r] >> (24 - i)) & 0xFF, EA);
		if (PowerPC::ppcState.Exceptions & EXCEPTION_DSI)
		{
			return;
		}
...
{
	u32 uAddress = Helper_Get_EA_X(_inst);
	Memory::CPU_Write_U32(Common::swap32(rGPR[_inst.RS]), uAddress);
}
...
void Interpreter::tlbie(UGeckoInstruction _inst)
{
	...
	Memory::CPU_InvalidateTLBEntry(_Address);
}
