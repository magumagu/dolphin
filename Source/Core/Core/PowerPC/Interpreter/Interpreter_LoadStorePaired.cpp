...
void Interpreter::Helper_Quantize(const u32 _Addr, const double _fValue, const EQuantizeType _quantizeType, const unsigned int _uScale)
{
	...
	{
	case QUANTIZE_FLOAT:
		Memory::CPU_Write_U32(ConvertToSingleFTZ(*(u64*)&_fValue), _Addr);
		break;
	...
	case QUANTIZE_U8:
		{
			...
			Memory::CPU_Write_U8((u8)fResult, _Addr);
		}
		break;
	...
	case QUANTIZE_U16:
		{
			...
			Memory::CPU_Write_U16((u16)fResult, _Addr);
		}
		break;
	...
	case QUANTIZE_S8:
		{
			...
			Memory::CPU_Write_U8((u8)(s8)fResult, _Addr);
		}
		break;
	...
	case QUANTIZE_S16:
		{
			...
			Memory::CPU_Write_U16((u16)(s16)fResult, _Addr);
		}
		break;
...
float Interpreter::Helper_Dequantize(const u32 _Addr, const EQuantizeType _quantizeType, const unsigned int _uScale)
{
	...
	case QUANTIZE_FLOAT:
		{
			u32 dwValue = Memory::CPU_Read_U32(_Addr);
			fResult = *(float*)&dwValue;
		}
		break;

	case QUANTIZE_U8:
		fResult = static_cast<float>(Memory::CPU_Read_U8(_Addr)) * m_dequantizeTable[_uScale];
		break;

	case QUANTIZE_U16:
		fResult = static_cast<float>(Memory::CPU_Read_U16(_Addr)) * m_dequantizeTable[_uScale];
		break;

	case QUANTIZE_S8:
		fResult = static_cast<float>((s8)Memory::CPU_Read_U8(_Addr)) * m_dequantizeTable[_uScale];
		break;

		// used for THP player
	case QUANTIZE_S16:
		fResult = static_cast<float>((s16)Memory::CPU_Read_U16(_Addr)) * m_dequantizeTable[_uScale];
		break;
