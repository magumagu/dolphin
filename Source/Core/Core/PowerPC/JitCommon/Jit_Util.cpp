...
#endif
	{
		...
		if (opAddress.IsImm())
		{
			...
			else
			{
				...
				{
				case 32:
					ABI_CallFunctionAC((void *)&Memory::Read_U32_Val, R(reg_value), address);
					break;
				case 16:
					if (signExtend)
						ABI_CallFunctionAC((void *)&Memory::Read_U16_ZX_Val, R(reg_value), address);
					else
						ABI_CallFunctionAC((void *)&Memory::Read_U16_SX_Val, R(reg_value), address);
					break;
				case 8:
					if (signExtend)
						ABI_CallFunctionAC((void *)&Memory::Read_U8_ZX_Val, R(reg_value), address);
					else
						ABI_CallFunctionAC((void *)&Memory::Read_U8_SX_Val, R(reg_value), address);
					break;
				}
		...
		else
		{
			...
			{
			case 32:
				ABI_CallFunctionAA((void *)&Memory::Read_U32_Val, R(reg_value), addr_loc);
				break;
			case 16:
				if (signExtend)
					ABI_CallFunctionAA((void *)&Memory::Read_U16_SX_Val, R(reg_value), addr_loc);
				else
					ABI_CallFunctionAA((void *)&Memory::Read_U16_ZX_Val, R(reg_value), addr_loc);
				break;
			case 8:
				if (signExtend)
					ABI_CallFunctionAA((void *)&Memory::Read_U8_SX_Val, R(reg_value), addr_loc);
				else
					ABI_CallFunctionAA((void *)&Memory::Read_U8_ZX_Val, R(reg_value), addr_loc);
