...
static bool InstallCodeHandler()
{
	...
	{
		// Patch MMIO address
		if (Memory::Debug_Read_U32(INSTALLER_BASE_ADDRESS + h) == (0x3f000000u | ((mmioAddr ^ 1) << 8)))
		{
			NOTICE_LOG(ACTIONREPLAY, "Patching MMIO access at %08x", INSTALLER_BASE_ADDRESS + h);
			Memory::Debug_Write_U32(0x3f000000u | mmioAddr << 8, INSTALLER_BASE_ADDRESS + h);
		}
	}
	...
	Memory::Debug_Write_U32(0xd01f1bad, INSTALLER_BASE_ADDRESS);

	// Create GCT in memory
	Memory::Debug_Write_U32(0x00d0c0de, codelist_base_address);
	Memory::Debug_Write_U32(0x00d0c0de, codelist_base_address + 4);
	...
	for (const GeckoCode& active_code : active_codes)
	{
		if (active_code.enabled)
		{
			for (const GeckoCode::Code& code : active_code.codes)
			{
				...
				{
					Memory::Debug_Write_U32(code.address, codelist_base_address + 8 + i);
					Memory::Debug_Write_U32(code.data, codelist_base_address + 12 + i);
					i += 8;
				}
			}
		}
	...
	Memory::Debug_Write_U32(0xff000000, codelist_base_address + 8 + i);
	Memory::Debug_Write_U32(0x00000000, codelist_base_address + 12 + i);

	// Turn on codes
	Memory::Debug_Write_U8(1, INSTALLER_BASE_ADDRESS + 7);
...
void RunCodeHandler()
{
	...
	{
		if (!code_handler_installed || Memory::Debug_Read_U32(INSTALLER_BASE_ADDRESS) - 0xd01f1bad > 5)
