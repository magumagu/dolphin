...
static bool InstallCodeHandler()
{
	...
	for (size_t i = 0, e = data.length(); i < e; ++i)
		PowerPC::Debug_Write_U8(data[i], (u32)(INSTALLER_BASE_ADDRESS + i));

	// Patch the code handler to the system starting up
	for (unsigned int h = 0; h < data.length(); h += 4)
	{
		// Patch MMIO address
		if (PowerPC::Debug_Read_U32(INSTALLER_BASE_ADDRESS + h) == (0x3f000000u | ((mmioAddr ^ 1) << 8)))
		{
			NOTICE_LOG(ACTIONREPLAY, "Patching MMIO access at %08x", INSTALLER_BASE_ADDRESS + h);
			PowerPC::Debug_Write_U32(0x3f000000u | mmioAddr << 8, INSTALLER_BASE_ADDRESS + h);
		}
	}
	...
	PowerPC::Debug_Write_U32(0xd01f1bad, INSTALLER_BASE_ADDRESS);

	// Create GCT in memory
	PowerPC::Debug_Write_U32(0x00d0c0de, codelist_base_address);
	PowerPC::Debug_Write_U32(0x00d0c0de, codelist_base_address + 4);
	...
	for (const GeckoCode& active_code : active_codes)
	{
		if (active_code.enabled)
		{
			for (const GeckoCode::Code& code : active_code.codes)
			{
				...
				{
					PowerPC::Debug_Write_U32(code.address, codelist_base_address + 8 + i);
					PowerPC::Debug_Write_U32(code.data, codelist_base_address + 12 + i);
					i += 8;
				}
			}
		}
	...
	PowerPC::Debug_Write_U32(0xff000000, codelist_base_address + 8 + i);
	PowerPC::Debug_Write_U32(0x00000000, codelist_base_address + 12 + i);

	// Turn on codes
	PowerPC::Debug_Write_U8(1, INSTALLER_BASE_ADDRESS + 7);
...
void RunCodeHandler()
{
	...
	{
		if (!code_handler_installed || PowerPC::Debug_Read_U32(INSTALLER_BASE_ADDRESS) - 0xd01f1bad > 5)
