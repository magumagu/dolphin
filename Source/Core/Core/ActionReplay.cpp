...
static bool Subtype_RamWriteAndFill(const ARAddr& addr, const u32 data)
{
	...
	case DATATYPE_8BIT:
	{
		...
		{
			Memory::Debug_Write_U8(data & 0xFF, new_addr + i);
	...
	case DATATYPE_16BIT:
	{
		...
		for (u32 i = 0; i <= repeat; ++i)
		{
			Memory::Debug_Write_U16(data & 0xFFFF, new_addr + i * 2);
	...
	case DATATYPE_32BIT: // Dword write
		...
		LogInfo("--------");
		Memory::Debug_Write_U32(data, new_addr);
...
{
	const u32 new_addr = addr.GCAddress();
	const u32 ptr = Memory::Debug_Read_U32(new_addr);
	...
	case DATATYPE_8BIT:
	{
		...
		Memory::Debug_Write_U8(thebyte, ptr + offset);
	...
	case DATATYPE_16BIT:
	{
		...
		LogInfo("Offset: %08x", offset);
		Memory::Debug_Write_U16(theshort, ptr + offset);
	...
	case DATATYPE_32BIT:
		...
		LogInfo("--------");
		Memory::Debug_Write_U32(data, ptr);
...
static bool Subtype_AddCode(const ARAddr& addr, const u32 data)
{
	...
	case DATATYPE_8BIT:
		LogInfo("8-bit Add");
		LogInfo("--------");
		Memory::Debug_Write_U8(Memory::Debug_Read_U8(new_addr) + data, new_addr);
		LogInfo("Wrote %08x to address %08x", Memory::Debug_Read_U8(new_addr) + (data & 0xFF), new_addr);
		LogInfo("--------");
		break;
	...
	case DATATYPE_16BIT:
		LogInfo("16-bit Add");
		LogInfo("--------");
		Memory::Debug_Write_U16(Memory::Debug_Read_U16(new_addr) + data, new_addr);
		LogInfo("Wrote %08x to address %08x", Memory::Debug_Read_U16(new_addr) + (data & 0xFFFF), new_addr);
		LogInfo("--------");
		break;
	...
	case DATATYPE_32BIT:
		LogInfo("32-bit Add");
		LogInfo("--------");
		Memory::Debug_Write_U32(Memory::Debug_Read_U32(new_addr) + data, new_addr);
		LogInfo("Wrote %08x to address %08x", Memory::Debug_Read_U32(new_addr) + data, new_addr);
		LogInfo("--------");
		break;
...
static bool ZeroCode_FillAndSlide(const u32 val_last, const ARAddr& addr, const u32 data) // This needs more testing
{
	...
	case DATATYPE_8BIT:
		...
		{
			Memory::Debug_Write_U8(val & 0xFF, curr_addr);
			curr_addr += addr_incr;
			val += val_incr;
	...
	case DATATYPE_16BIT:
		...
		for (int i=0; i < write_num; ++i)
		{
			Memory::Debug_Write_U16(val & 0xFFFF, curr_addr);
	...
	case DATATYPE_32BIT:
		...
		for (int i = 0; i < write_num; ++i)
		{
			Memory::Debug_Write_U32(val, curr_addr);
...
static bool ZeroCode_MemoryCopy(const u32 val_last, const ARAddr& addr, const u32 data)
{
	...
	if ((data & ~0x7FFF) == 0x0000)
	{
		...
		{ // Memory Copy With Pointers Support
			...
			for (int i = 0; i < 138; ++i)
			{
				Memory::Debug_Write_U8(Memory::Debug_Read_U8(addr_src + i), addr_dest + i);
				LogInfo("Wrote %08x to address %08x", Memory::Debug_Read_U8(addr_src + i), addr_dest + i);
			}
			LogInfo("--------");
		}
		else
...
static bool ConditionalCode(const ARAddr& addr, const u32 data, int* const pSkipCount)
{
	...
	{
	case DATATYPE_8BIT:
		result = CompareValues((u32)Memory::Debug_Read_U8(new_addr), (data & 0xFF), addr.type);
		break;

	case DATATYPE_16BIT:
		result = CompareValues((u32)Memory::Debug_Read_U16(new_addr), (data & 0xFFFF), addr.type);
		break;

	case DATATYPE_32BIT_FLOAT:
	case DATATYPE_32BIT:
		result = CompareValues(Memory::Debug_Read_U32(new_addr), data, addr.type);
		break;
