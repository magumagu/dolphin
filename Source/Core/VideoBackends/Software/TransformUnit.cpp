...
{
	const float* mat = (const float*)&xfmem[XFMEM_POSMATRICES + src->posMtx * 4];
...
{
	const float* mat = (const float*)&xfmem[XFMEM_NORMALMATRICES + (src->posMtx & 31) * 3];
...
void TransformTexCoordRegular(const TexMtxInfo &texinfo, int coordNum, bool specialCase, const InputVertexData *srcVertex, OutputVertexData *dstVertex)
{
	...
	const float *mat = (const float*)&xfmem[XFMEM_POSMATRICES + srcVertex->texMtx[coordNum] * 4];
	...
	if (xfregs.dualTexTrans.enabled)
	{
		...
		const float *postMat = (const float*)&xfmem[XFMEM_POSTMATRICES + postInfo.index * 4];
...
{
	const LightPointer *light = (const LightPointer*)&xfmem[XFMEM_LIGHTS + 0x10 * lightNum];
...
{
	for (u32 chan = 0; chan < swxfregs.numChan.numColorChans; chan++)
	{
		// abgr
		u8 matcolor[4];
		u8 chancolor[4];
...
{
	for (u32 coordNum = 0; coordNum < swxfregs.numTexGen.numTexGens; coordNum++)
	{
		...
		case XF_TEXGEN_EMBOSS_MAP:
			{
				const LightPointer *light = (const LightPointer*)&xfmem[XFMEM_LIGHTS + 0x10 * texinfo.embosslightshift];
